...
def najdi_besede(besedilo, podniz):
    vzorec = re.compile('\w*' + podniz + '\w*')
    iterator = vzorec.finditer(besedilo)
    mn = set()
    [mn.add(beseda.group(0)) for beseda in iterator]
    return mn
...
def najdi_predpono(besedilo, predpona):
    vzorec = re.compile('\W(' + predpona + '\w*)')
    print(vzorec)
    iterator = vzorec.finditer(besedilo)
    mn = set()
    [mn.add(i.group(1)) for i in iterator]
    return mn
...
def podvojene_crke(besedilo):
    vzorec = re.compile(r'\w*(\w)\1\w*')
    iterator = vzorec.finditer(besedilo)
    mn = set()
    [mn.add(i.group(0)) for i in iterator]
    return mn
